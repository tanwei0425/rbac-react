...
import request from '@/services/element';
...
const Index = () => {
    ...
    const detailsRequestRes = useRequest(request.getElementDetail, { manual: true });
    const tableRequestRes = useRequest(request.getElementTable, { manual: true });
    const createRequestRes = useRequest(request.createElement, { manual: true });
    const updateRequestRes = useRequest(request.updateElement, { manual: true });
    const deleteRequestRes = useRequest(request.deleteElement, { manual: true });
    ...
    const getMenuListData = async () => {
        ...
        if (res?.code === 200) {
            ...
            const topMenuTreeData = [{ value: 0, title: '公共元素' }, ...menuTreeData];
